...
class CCNetworkController extends Controller {
    ...
    public function add(Request $request) {
		...
		if($validator->fails()) {
			...
			// message
			$this->_apiData['message'] = trans('system.invalid_user_account');
		} elseif($user->status == 0){
			// kick user
			$this->_apiData['kick_user'] = 1;
			// message
			$this->_apiData['message'] = trans('system.account_is_inactive');
		} elseif($user->deleted_at !== NULL || $user->status > 1){
			// kick user
			$this->_apiData['kick_user'] = 1;
			// message
			$this->_apiData['message'] = trans('system.user_deleted_or_baned');
